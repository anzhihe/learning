...
Select * from Student Where SName between '王进' and '陈鹏' 
Select * from Student order by SName

-- ================================================
-- 1. 查询出手机号码133或者134开头，倒数第四位为不是2也不是4的学生姓名
Select * 
from Student 
where MobileNo REGEXP '^[1][3][34][0-9]{1,}[^24][0-9]$'
# where MobileNO Like '133%'  -- [] _

-- 2. 对Student表按照年龄升序排序，如果年龄一样，女生排在男生前面 
Select * from Student 
order by Sage ASC , Sex ASC
...
-- =========================分组查询=================
-- 分组里面在mysql中， GROUP_CONCAT(expr)
Select Sex, GROUP_CONCAT(sname) As '学生姓名', Count(*) AS '人数', avg(sage) As '平均年龄',sum(sage) As '年龄和'
from Student
Group by Sex
...
-- SQL99标准（SQL92标准中只有内连接、外连接、交叉连接、自连接。。。）
...
Select T1.BookID As '图书编号', BookName As '图书名称', BookIncoming As '入存量',
       Ifnull(Number, 0) As '被借量', (BookIncoming - Ifnull(Number, 0)  ) As '库存量'
from Book As T1
Left Outer Join 
(
...
Select T1.SNo As '学号', T2.SName As '姓名',T1.语文,T1.数学,T1.英语, T1.物理,T1.化学,T1.总分,T1.均分 
from
(
	Select  SNO,
		...
		SUM(result) As '总分',
		ROUND(SUM(Result)*1.0/5,2) As '均分'
	from Score 
	Group By SNo
