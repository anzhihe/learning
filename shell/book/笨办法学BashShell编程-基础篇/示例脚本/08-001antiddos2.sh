...
do
  ...
  for ip in `lsof -ni | grep $service_name | grep -iv listen | eval "awk '{print \$$filed}'" | cut -d : -f 2 | sort | uniq | sed s/"http->"//`
  do
    # 查找此特定IP地址的连接数
    noconns=`lsof -ni | grep $ip | wc -l`;
    echo $ip : $noconns ;

    if [ "$noconns" -gt "$max_connection" ] ;  then
      # 如果建立的连接数超过阈值
      # 写日志
      ...
      echo `date` "$ip has $noconns connections.  Total connections to prod spider:  `lsof -ni | grep nginx | grep -iv listen | wc -l`" >> $log_file
